...
`scopes` (default shown), `token_lifetime_seconds` (600–3600, default 3600) and `identity_proof` (`oidc`, the default, or `aws_sigv4`) are optional. `audience` must be a pool provider resource name in the form shown and `service_account` an email address. `identity_proof` names the kind of workload identity pool provider `audience` is, and so how Portunus proves the federation session's identity to Google STS. Steps 1 and 2 are as for `anthropic_wif`; then Portunus:

3. Assumes the federation role with the caller's own credentials, as above, and proves that session's identity. With `oidc`, the session requests an STS web identity token for `audience`, RS256-signed, 15 minutes long and carrying the four tags, exactly as for `anthropic_wif`. With `aws_sigv4`, no STS web identity token is issued: Portunus signs an AWS `GetCallerIdentity` request for `sts.<region>.amazonaws.com` with the session's credentials (SigV4, using [google-auth](https://github.com/googleapis/google-auth-library-python)'s request signer and the SDK's configured region, `AWS_DEFAULT_REGION`, which must be set for this proof), bound to `audience` through a signed `x-goog-cloud-target-resource` header.
4. Exchanges the proof at `https://sts.googleapis.com/v1/token` (`subject_token_type` `urn:ietf:params:oauth:token-type:jwt` for `oidc`, `urn:ietf:params:aws:token-type:aws4_request` for `aws_sigv4`) for a federated token for `audience`, then calls `https://iamcredentials.googleapis.com/v1/projects/-/serviceAccounts/<service_account>:generateAccessToken` with `scopes` and `token_lifetime_seconds`. The access token is returned with `output_header: "authorization"` and `output_prefix: "Bearer "`.

As for `anthropic_wif`, an unreachable Google endpoint, a 5xx/429 answer or a missed 6 s deadline returns 503.

On the Google side, all deployment concerns. With `oidc`, the pool provider is an OIDC provider whose issuer is the federation role's account's STS issuer URL and whose allowed audience is the provider's own resource name, the secret's `audience`. The token's `sub` is the federation role's IAM ARN including its path (`federation_role_arn`), so the attribute mapping derives `google.subject` from `assertion.sub` with an `extract` that keeps it within Google's 127-character subject limit, and the attribute condition matches the role path prefix. The four Portunus tags arrive as `request_tags` under the `https://sts.amazonaws.com/` claim and are available to the attribute mapping, e.g. `assertion['https://sts.amazonaws.com/']['request_tags']['portunus:user']`. The federation role's identity policy must allow `sts:GetWebIdentityToken` for `audience` and `sts:TagGetWebIdentityToken` for the four tag keys, as for the other minted types (see [Caching and the federation role](#caching-and-the-federation-role)). With `aws_sigv4`, the pool provider is Google's AWS type for the federation role's account. Google replays the signed request against AWS itself, so the federation role needs no IAM permissions, and the provider's attribute condition sees only the assumed-role ARN (`arn:aws:sts::<account>:assumed-role/<role name>/<caller role name>`, which drops the IAM path); no tags reach Google. Either way the service account must grant `roles/iam.workloadIdentityUser` to the pool principal the mapping produces.
