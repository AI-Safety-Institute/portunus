...
- **Portunus backend**. A pure gRPC process (`python -m portunus.grpc.server` — FastAPI was retired in the 0.6.0 cutover) hosting the two servicers above plus the standard `grpc.health.v1.Health` and reflection services. Envoy answers `/ping` (process liveness) and `/healthz` (gated on Portunus's gRPC health — the ALB health-check target); flushing the shared auth cache is an operator runbook — see [`docs/runbooks/flush-auth-cache.md`](docs/runbooks/flush-auth-cache.md) — not an HTTP endpoint:
