...
This file groups the behaviours that all share the same assertion shape:
*"send an HTTP request, assert on the status code, whether the upstream
saw it, and whether the response body matches expectations."* That set of
behaviours is large enough to benefit from a parameterised corpus driver
(``test_request_response``), so we don't write a new pytest function for
each one.

**This is one test family, not the whole test suite.** Other behaviour
families have different assertion shapes and live in their own test files:

* Streaming/chunked responses — need assertions on chunk arrival timing.
* Telemetry — assertions against Kinesis records via LocalStack.
* Cache TTL / Redis-down — time-sensitive, fixture controls state.
* WebSocket — separate fixture, frame-level assertions (see CLAUDE: "I
  don't mind using different fixtures for web sockets").
* Drain / lifecycle — needs container restart, not just HTTP traffic.
* Inspect / Codex / Claude Code — Inspect ``@task`` definitions, scorer
  assertions on agent transcripts.

The corpus and the runner are co-located. Split into ``scenarios.py`` if it
gets unwieldy to read in one file.
"""
...
# Kinesis stream the metadata-flagged records land in (see Portunus's
# KINESIS_METADATA_STREAM env var in docker-compose.yaml).
_METADATA_STREAM = "portunus-stream-metadata"


def _read_localstack_metadata_records() -> list[dict]:
    """Read all metadata records from LocalStack Kinesis.

    LocalStack-only — uses ``docker exec localstack-main awslocal kinesis ...``
    rather than a real boto3 client so the test runner doesn't need any
    AWS credentials or boto3 setup. Mirrors the helper already in
    ...
    if records_result.returncode != 0:
        return []
    payload = json.loads(records_result.stdout)
    return [json.loads(base64.b64decode(r["Data"])) for r in payload.get("Records", [])]
...
SCENARIOS: list[Scenario] = [
    ...
    # ---- Security: header forgery (expected to fail until task #47 lands) -
    # Today the proxy forwards the client's x-portunus-target-host through
    # to ext_authz, where it overrides the host-validation. This scenario
    # encodes the *desired* behaviour: a client cannot bypass host
    # validation by forging the header. It should fail today (status=200,
    # upstream reached) and start passing after the fix lands.
    Scenario(
...
@pytest.mark.parametrize(
    "scenario",
    [s for s in SCENARIOS if s.expected.expected_metadata_records is not None],
    ids=lambda s: s.name,
)
def test_metadata_telemetry(
    ...
    """Each scenario produces the expected number of metadata records.

    Snapshot the Kinesis metadata stream before, fire the scenario's
    request, then poll until either the expected delta appears or a
    short timeout fires. Asserts on the delta rather than the absolute
    count so prior tests' records don't interfere.
    """
    baseline = len(_read_localstack_metadata_records())
