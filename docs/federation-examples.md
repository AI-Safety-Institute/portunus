...
6. **Identity proof.** Anthropic, OpenAI, OpenRouter, and GCP with `identity_proof` `oidc` (the default): `GetWebIdentityToken` from the federation session for the secret's `audience`, tagged with the four tag keys. GCP with `identity_proof` `aws_sigv4`: a SigV4-signed `GetCallerIdentity` request bound to the pool provider. Permissions in [The AWS side](#the-aws-side); parameters in each provider's *What Portunus sends*.
...
```text
portunus encode-credentials SECRET_ARN [--policy FILE_OR_JSON] [--federation-role-path /portunus-fed/] [--session-name portunus]
```
...
Parameters:
  ...
  ProviderAudience:
    ...
    Description: The secret's audience. Unused by gcp_wif with identity_proof aws_sigv4.
...
- `gcp_wif` with `identity_proof` `aws_sigv4` needs no identity policy: the proof is a signed `GetCallerIdentity`, which requires no permission. Drop `Policies`; keep the trust policy and boundary. With the default `oidc` proof the role needs `IssueIdentityToken` as above, with `ProviderAudience` set to the pool provider resource name.
...
`audience` must match `//iam.googleapis.com/projects/<number>/locations/global/workloadIdentityPools/<pool>/providers/<provider>`; `service_account` is an email; `scopes` (default shown, at least one), `token_lifetime_seconds` (600–3600, default 3600) and `identity_proof` (`oidc`, the default, or `aws_sigv4`) are optional. `identity_proof` names the kind of pool provider `audience` is: an OIDC provider fed by the account's STS issuer (`oidc`), or Google's AWS provider type (`aws_sigv4`). Portunus needs `AWS_DEFAULT_REGION` set for `aws_sigv4`.

### Provider configuration

One pool, with a provider of either kind. The member in the `roles/iam.workloadIdentityUser` binding uses the project number. The service account also needs the roles the upstream API requires (`roles/aiplatform.user` for Vertex AI).

**OIDC provider (`identity_proof` `oidc`).** Google verifies the STS web identity token against the STS issuer URL's JWKS. `--allowed-audiences` must be set: the token's `aud` is the secret's `audience`, `//iam.googleapis.com/…`, and without the flag Google expects `https://iam.googleapis.com/…`. `sub` is the federation role's IAM ARN with its path, `arn:aws:iam::123456789012:role/portunus-fed/teams/example-team/portunus-fed-example-grant@teams.example-team`, which a longer path can push past the 127-character limit on `google.subject`, so the mapping keeps the part after `role/portunus-fed/` and the attribute condition matches the role path. The four tags sit under the `https://sts.amazonaws.com/` claim as `request_tags`, so the caller can be kept in an attribute:
...
The federation role needs the `IssueIdentityToken` policy from [The AWS side](#the-aws-side) with `ProviderAudience` set to the pool provider resource name.

**AWS provider (`identity_proof` `aws_sigv4`).** Google sees the assumed-role ARN `arn:aws:sts::123456789012:assumed-role/portunus-fed-example-grant@teams.example-team/<caller role name>`: no IAM path, and the session segment is the caller's role name. Map `google.subject` to the role name and keep the caller in an attribute:
...
| 403 `Google STS exchange for <sa> returned HTTP 400` | Attribute condition false; `audience` ≠ provider resource name; `identity_proof` does not match the provider's kind. OIDC provider: `iss` ≠ `--issuer-uri`, `aud` not in `--allowed-audiences`, mapped `google.subject` over 127 characters. AWS provider: `--account-id` ≠ `123456789012`; signed request rejected on replay | Portunus log line with the response body |
