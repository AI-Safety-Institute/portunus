...
def _build_default_policy(
    ...
    Grants GetSecretValue on ``secret_arn`` and, when the secret's name places
    it in a namespace, sts:AssumeRole on the federation roles of that namespace
    under ``federation_role_path`` in the caller's account.
    """
...
def encode_credentials(
    ...
    Args:
        ...
        policy: Optional IAM session policy JSON string. If None, uses the
            ...
            the federation roles in the secret's namespace).
