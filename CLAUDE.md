...
3. Portunus (`app.py` → `services.auth_service` → `AuthService.get_api_key_from_payload`):
   ...
   - Retrieves the secret from AWS Secrets Manager and parses it (`services.secret_validation_service.parse_secret`): plaintext or `{"secret", "host", "signing_key"}` is a stored key; `{"type": "anthropic_wif", ...}` or `{"type": "gcp_workload_identity", ...}` describes a token to mint
   - For mint secrets, `services.federation_service.TokenMintService` checks the federation role ARN against `FEDERATION_ALLOWED_ACCOUNT_IDS` / `FEDERATION_ROLE_PATH_PREFIX` and assumes the role with the caller's credentials (regional STS endpoint). `anthropic_wif`: issues an STS web identity token from that session and exchanges it at `https://<host>/v1/oauth/token`. `gcp_workload_identity`: google-auth signs an AWS `GetCallerIdentity` request with the session credentials (region from `AWS_DEFAULT_REGION`), exchanges it at Google STS and impersonates the service account; the blocking refresh runs in a worker thread. Either way the result carries `output_header="authorization"`, `output_prefix="Bearer "`. Concurrent misses for one payload share a mint per process.
