...
- `FEDERATION_ATTRIBUTION_TAG_KEY`: Session tag key carrying the attribution handle for secrets with `attribution: pseudonymous` (default: "portunus:attributed_to")
- `FEDERATION_ATTRIBUTION_KEY`: HMAC key the attribution handle is computed under (a random secret of at least 32 bytes; needed only when a secret uses `pseudonymous`)
